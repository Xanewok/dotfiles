...
# macOS without developer tools ships /usr/bin/git as a shim: `command -v git`
# succeeds, but ANY invocation (even `git --version`) can pop the CLT install
# dialog — a prompt render must never do that. `xcode-select -p` probes the CLT
